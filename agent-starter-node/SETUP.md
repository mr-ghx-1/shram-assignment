...
5. Ensure you have access to the Realtime API (gpt-4o-realtime-preview)

**About the Realtime API:**
The OpenAI Realtime API provides an integrated pipeline for speech-to-text, language understanding, and text-to-speech in a single low-latency API call. This eliminates the need for separate STT (like Deepgram) and TTS services.
...
The agent logs detailed latency metrics for the voice pipeline:

- End-to-end latency (OpenAI Realtime API integrated pipeline)
...
1. **Use efficient models**: gpt-4o-realtime-preview provides integrated STT+LLM+TTS
2. **Monitor usage**: Check OpenAI dashboard for Realtime API usage
3. **Implement caching**: Cache frequent queries (already implemented in Next.js API)
4. **Set usage limits**: Configure API key limits in OpenAI dashboard
5. **Agent reuse**: The system reuses agent instances to reduce cold start costs
...
- Check the [OpenAI Realtime API documentation](https://platform.openai.com/docs/guides/realtime)
- Review the [LiveKit OpenAI Plugin documentation](https://docs.livekit.io/agents/plugins/openai/)

...
export async function withRetry<T>(
  ...
  while (attempt <= opts.maxRetries) {
    try {
      ...
      if (attempt > 0) {
        logger.info(`[Retry] Success after ${attempt} retry(ies)`);
      }
      ...
      logger.warn(`[Retry] Attempt ${attempt + 1} failed:`, lastError.message);

      // Check if we should retry
      if (attempt >= opts.maxRetries) {
        logger.error(`[Retry] All ${opts.maxRetries + 1} attempts failed`);
        break;
      }

      // Check if error is retryable
      if (!opts.shouldRetry(lastError)) {
        logger.warn('[Retry] Error is not retryable, aborting');
        break;
      }
      ...
      logger.debug(`[Retry] Waiting ${delay}ms before retry ${attempt + 2}/${opts.maxRetries + 1}`);
      await sleep(delay);

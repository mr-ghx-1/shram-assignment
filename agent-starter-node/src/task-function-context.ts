...
export class TaskFunctionContext {
  ...
  private currentFilter: { query?: string; priority?: number; scheduled?: string; completed?: boolean; tags?: string[] } | null = null;
  ...
  constructor(room?: any) {
    ...
    logger.info(`TaskFunctionContext initialized with API base URL: ${this.apiBaseUrl}`);
  }
  ...
  private async sendDataToFrontend(data: { type: string; payload?: any }) {
    ...
    try {
      ...
      logger.debug('Sent data to frontend:', data.type);
    } catch (err) {
      logger.warn('Failed to send data to frontend:', err);
    }
  }
    ...
    return withRetry(
      async () => {
        ...
        if (!response.ok) {
          ...
          logger.error(`HTTP ${response.status} error from ${url}:`, errorBody);
  ...
  private async resolveTaskIdentifier(identifier: string): Promise<string> {
    logger.debug(`Resolving task identifier: "${identifier}"`);
    ...
    logger.debug(`Using ${usingFilteredTasks ? 'filtered' : 'all'} tasks (${tasks.length} tasks)`);
    ...
    if (tasks.length === 1 && vagueIdentifiers.includes(identifier.toLowerCase().trim())) {
      const task = tasks[0];
      if (task) {
        logger.debug(`Only one task visible, resolving vague identifier "${identifier}" to: ${task.id} - "${task.title}"`);
        return task.id;
      }
    }
    ...
    if (plainNumberMatch) {
      ...
      logger.debug(`Parsed plain number reference: position ${index + 1}`);
      ...
      logger.debug(`Resolved to task: ${task.id} - "${task.title}"`);
      return task.id;
    }
    ...
    if (ordinalMatch && ordinalMatch[1]) {
      ...
      logger.debug(`Parsed ordinal reference: position ${index + 1}`);
    ...
    logger.debug('Attempting semantic matching on task titles');
    ...
    logger.debug(`Found ${matches.length} matching tasks`);
      ...
      try {
        ...
        if (scheduled_time) {
          try {
            ...
            logger.debug(`Parsed date "${scheduled_time}" to ${parsedDate}`);
          } catch (dateError) {
        ...
        if (priorityValue !== null) {
          ...
          logger.debug(`Parsed priority "${priority}" to ${priorityValue}`);
          ...
          logger.debug('No priority specified, defaulting to 1 (lowest)');
        }
  ...
  get_tasks = llm.tool({
    ...
    parameters: z.object({
      ...
      completed: z.boolean().nullish().describe('Filter by completion status: true for completed tasks, false for incomplete tasks. Leave empty for all tasks.'),
      tags: z.array(z.string()).nullish().describe('Filter by tags (e.g., ["work", "urgent"]). Leave empty for all tasks.'),
    }),
    execute: async ({ query, priority, scheduled, completed, tags }) => {
      try {
        logger.info('Fetching tasks with filters:', { query, priority, scheduled, completed, tags });
        ...
        if (priorityValue !== null) {
          ...
          logger.debug(`Parsed priority filter "${priority}" to ${priorityValue}`);
        } else if (priority !== undefined && priority !== null) {
          // Invalid priority provided
        ...
        if (scheduled && scheduled !== 'all' && scheduled.trim() !== '') {
          ...
          if (isoDateRegex.test(scheduled)) {
            ...
            logger.warn(`Invalid scheduled date format: "${scheduled}"`);
          }
        }
        ...
        // Only include non-null/undefined values in payload
        const payload: any = {};
        if (query) payload.query = query;
        if (priorityValue !== null) payload.priority = priorityValue;
        if (scheduled) payload.scheduled = scheduled;
        if (completed !== undefined && completed !== null) payload.completed = completed;
        ...
        if (tasks.length === 0) {
          if (query || priority || scheduled || completed !== undefined || (tags && tags.length > 0)) {
  ...
  update_task = llm.tool({
    ...
    parameters: z.object({
      ...
      completed: z
        ...
        .nullish()
        .describe('Completion status (true/false)')
        .transform((val) => {
    ...
    execute: async ({ identifier, title, scheduled_time, priority, completed }) => {
      try {
        logger.info(`Updating task: "${identifier}"`, {
          title,
          scheduled_time,
          priority,
          completed,
  ...
  delete_task = llm.tool({
    ...
    execute: async ({ identifier }) => {
      try {
        logger.info(`Deleting task: "${identifier}"`);
        ...
        logger.info(`Task deleted successfully: ${task.id}`);
   ...
   * Provides the current date and time in the user's timezone
   */
  get_current_time = llm.tool({
    description: 'Get the current date and time in the user\'s timezone. Use this to determine what "today", "tomorrow", or relative dates mean.',
    parameters: z.object({}),
    execute: async () => {
      try {
        const now = new Date();
        ...
        const response = {
          ...
          readable_date: localDate,
          tomorrow_date: tomorrowISO,
          day_after_tomorrow_date: dayAfterTomorrowISO,
          day_of_week: now.toLocaleDateString('en-US', { weekday: 'long' }),
        };
        
        logger.info('Current time requested:', response);
        
        // Return as JSON string for the LLM to parse

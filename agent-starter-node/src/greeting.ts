...
export async function playGreeting(
  ...
  try {
    logger.debug('Checking if greeting should be played...');
    ...
    if (roomMetadata) {
      try {
        ...
        logger.warn('Failed to parse room metadata:', err);
      }
    }
    
    if (greetingPlayed) {
      logger.debug('Greeting already played for this room, skipping');
      return;
    }
    
    logger.info('Playing greeting message:', GREETING_CONFIG.message);
    ...
    logger.error('Failed to play greeting:', error);
